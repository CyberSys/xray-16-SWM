...
{
    //С‚РµРєСѓС‰РµРµ СЃРѕСЃС‚РѕСЏРЅРёРµ РІРµС‰Рё
    m_fCondition = 1.0f;
...
CSE_ALifeItemWeapon::CSE_ALifeItemWeapon(LPCSTR caSection) : CSE_ALifeItem(caSection)
{
    ...
    m_muzzle_section = NULL;
    m_launcher_section = NULL;
    m_magaz_section = NULL;
    m_spec_1_section = NULL;
    m_spec_2_section = NULL;
    m_spec_3_section = NULL;
    m_spec_4_section = NULL;
    ...
    m_grenade_launcher_status =
        (EWeaponAddonStatus)(READ_IF_EXISTS(pSettings, r_s32, s_name, "grenade_launcher_status", 0));
...
void CSE_ALifeItemWeapon::UPDATE_Read(NET_Packet& tNetPacket)
{
    ...
    // SM_TODO Сохранение типов патронов <!>
    /*
    m_AmmoIDs.clear();
...
{
    // SM_TODO: Тут и везде чтение версий сделай, чтобы совместимость с all.spawn и СДК не пофигачить !!!
    // if (m_wVersion > 40)
...
void CSE_ALifeItemWeapon::FillProps(LPCSTR pref, PropItemVec& items)
{
    ...
    // SM_TODO: Новые аддоны, m_addon_flags уже не существует
    /*
...
void CSE_ALifeItemWeaponMagazined::UPDATE_Read(NET_Packet& P) { inherited::UPDATE_Read(P); }
...
void CSE_ALifeItemWeaponMagazinedWGL::UPDATE_Read(NET_Packet& P) { inherited::UPDATE_Read(P); }
