...
public:
    ...
    struct launch_points
    {
        ...
        u32 dwLifeTime3p; // Время (в м\сек) после которого удалим гильзу (0 - не удаляем, для мировых гильз)
        u32 dwLifeTimeHud; // Время (в м\сек) после которого удалим гильзу (0 - не удаляем, для худовых гильз)

        bool m_bDestroyOnCollide3p; // Уничтожить гильзу при колллизии с геометрией (для мировых гильз)
        u32 dwMinCollideLifeTime3p; // Время (в м\сек) раньше которого не удаляем гильзу при столкновениях (для мировых гильз)
        bool m_bDestroyOnCollideHud; // Уничтожить гильзу при колллизии с геометрией (для худовых гильз)
        u32 dwMinCollideLifeTimeHud; // Время (в м\сек) раньше которого не удаляем гильзу при столкновениях (для худовых гильз)
