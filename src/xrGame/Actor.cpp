...
void CActor::On_SetEntity()
{
    ...
    /*
       При каждой загрузке игры удаляем все загруженные худовые модели предметов, т.к. g_player_hud
       не пересоздаётся при загрузке, и у моделей оружия могут остаться скрыте кости или другие
       присоединённые модели от предыдущего сохранения. Это нужно из-за апгрейдов, т.к. они могут менять
       возможные аддоны оружия на ходу - и при загрузке (в рамках одной игровой сессии)
       прошлого сохранения без этих апгрейдов: код управления аддонами не может корректно
       скрыть \ показать кости и модели, т.к. в конфиге самого оружия уже не прописаны связанные с ними аддоны.
       Аналогичная проблема бывает с двумя разными оружиями, но использующими одну худовую секцию.
       Возможно касается и мировых моделей оружия.
    */
    if (g_player_hud) //--#SM+#--
        g_player_hud->destroy_all_attached_items();
}
