...
{
    return pInput->iGetAsyncKeyState(DIK_LSHIFT) || pInput->iGetAsyncKeyState(DIK_Z) || pInput->iGetAsyncKeyState(DIK_X) ||
...
void attachable_hud_item::tune(Ivector values)
{
    ...
    // Сбросить в нули //--#SM+#--
    if (pInput->iGetAsyncKeyState(DIK_BACKSPACE))
    {
        bZeroMode = true;
    }
...
void player_hud::tune(Ivector _values) //--#SM+#--
{
    ...
    if (pInput->iGetAsyncKeyState(DIK_LSHIFT) && pInput->iGetAsyncKeyState(DIK_DELETE))
    {
        ...
        if (!pHudCfg)
            return;
        //-----------------//
        hi->m_measures.m_hands_attach[0] = READ_IF_EXISTS(pHudCfg,
            r_fvector3,
            sect_name,
            make_string("hands_position%s", (is_16x9) ? "_16x9" : "").c_str(),
            hi->m_measures.m_hands_attach[0]);
        hi->m_measures.m_hands_attach[1] = READ_IF_EXISTS(pHudCfg,
            r_fvector3,
            sect_name,
            make_string("hands_orientation%s", (is_16x9) ? "_16x9" : "").c_str(),
            hi->m_measures.m_hands_attach[1]);

        hi->m_measures.m_hands_offset[0][1] = READ_IF_EXISTS(pHudCfg,
            r_fvector3,
            sect_name,
            make_string("aim_hud_offset_pos%s", (is_16x9) ? "_16x9" : "").c_str(),
            hi->m_measures.m_hands_offset[0][1]);
        hi->m_measures.m_hands_offset[1][1] = READ_IF_EXISTS(pHudCfg,
            r_fvector3,
            sect_name,
            make_string("aim_hud_offset_rot%s", (is_16x9) ? "_16x9" : "").c_str(),
            hi->m_measures.m_hands_offset[1][1]);

        hi->m_measures.m_hands_offset[0][2] = READ_IF_EXISTS(pHudCfg,
            r_fvector3,
            sect_name,
            make_string("gl_hud_offset_pos%s", (is_16x9) ? "_16x9" : "").c_str(),
            hi->m_measures.m_hands_offset[0][2]);
        hi->m_measures.m_hands_offset[1][2] = READ_IF_EXISTS(pHudCfg,
            r_fvector3,
            sect_name,
            make_string("gl_hud_offset_rot%s", (is_16x9) ? "_16x9" : "").c_str(),
            hi->m_measures.m_hands_offset[1][2]);

        hi->m_measures.m_item_attach[0] = READ_IF_EXISTS(pHudCfg, r_fvector3, sect_name, "item_position", hi->m_measures.m_item_attach[0]);
...
void hud_draw_adjust_mode()
{
    ...
    if (pInput->iGetAsyncKeyState(DIK_LSHIFT) && hud_adj_mode)
        _text =
            "press SHIFT+NUM 0-return 1-hud_pos 2-hud_rot 3-itm_pos 4-itm_rot 5-fire_point 6-fire_2_point 7-shell_point 8-pos_step "
            "9-rot_step";
    ...
    if (_text) //--#SM+#--
    {
        ...
        if (cur_hud_name != NULL)
        {
            if (socket_idx != u8(-1))
                F->OutNext(
                    "for item [%s] idx = %d (%s)", cur_hud_name.c_str(), hud_adj_item_idx, (hud_adj_item_idx) ? "Detector" : "Attachment");
            else
                F->OutNext(
                    "for item [%s] idx = %d (%s)", cur_hud_name.c_str(), hud_adj_item_idx, (hud_adj_item_idx) ? "Detector" : "Weapon");
        }
        else
        ...
        if (hud_adj_item_idx == 0 && pInput->iGetAsyncKeyState(DIK_LCONTROL))
        {
