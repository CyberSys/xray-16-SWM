...
void CRender::add_leafs_Dynamic(dxRender_Visual* pVisual)
{
    ...
    case MT_HIERRARHY:
    {
        ...
        for (; I != E; I++) //--#SM+#--
        {
            (*I)->vis.obj_data = pV->getVisData().obj_data; // Наследники используют шейдерные данные от родительского
    ...
    case MT_SKELETON_RIGID:
    {
        ...
        else
        {
            ...
            for (; I != E; I++) //--#SM+#--
            {
                (*I)->vis.obj_data = pV->getVisData().obj_data; // Наследники используют шейдерные данные от
...
void CRender::add_leafs_Static(dxRender_Visual* pVisual)
{
    ...
    case MT_LOD:
    {
...
#endif
        {
            ...
            {
                (*I)->vis.obj_data =
                    pV->getVisData().obj_data; // Наследники используют шейдерные данные от родительского
                                               // визуала [use shader data from parent model, rather than
                                               // it childrens]
                add_leafs_Static(*I);
            }
